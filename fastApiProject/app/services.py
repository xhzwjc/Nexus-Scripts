...
class TaxCalculationService:
            ...
            with conn.cursor() as cursor:
                base_query = """
                             SELECT
                                 payment_over_time,
                                 bill_amount,
                                 DATE_FORMAT(payment_over_time, '%%Y-%%m') as year_months
                             FROM biz_balance_worker
                             WHERE deleted = 0
                                 AND pay_status = 3
                                 AND YEAR(payment_over_time) = %s
                               AND credential_num = %s \
                             """
                params = [year, credential_num]

                if realname:
                    base_query += " AND realname = %s"
                    params.append(realname)

                base_query += " ORDER BY payment_over_time"
                cursor.execute(base_query, tuple(params))
                results = cursor.fetchall()

                records = [
                    {'payment_time': row[0], 'bill_amount': float(row[1]), 'year_month': row[2]}
                    for row in results
                ]
    ...
    def calculate_income_amount(self, amount: float, income_type: int) -> float:
        """根据收入类型计算收入额"""
        if income_type == 1:  # 劳务报酬
            if amount <= 4000:
                return amount - 800
            return amount * 0.8
        else:  # 工资薪金
    ...
    def calculate_tax(self,
                      ...
                      accumulated_special_deduction: float = 0.0,
                      accumulated_additional_deduction: float = 0.0,  # 新增：专项附加扣除
                      accumulated_other_deduction: float = 0.0,  # 新增：其他扣除
                      accumulated_pension_deduction: float = 0.0,  # 新增：个人养老金
                      accumulated_donation_deduction: float = 0.0,
                      use_mock_data: bool = False,
                      mock_records: Optional[List[Dict]] = None) -> List[Dict[str, Any]]:
        """计算个人所得税"""
        if year is None:
        ...
        self.logger.info(
            f"开始计算{income_type_name}税款: 身份证={credential_num}, 年份={year}, 姓名={realname or '未提供'}")

        # 获取记录（数据库或模拟数据）
        if use_mock_data and mock_records:
            records = sorted(mock_records, key=lambda x: x['year_month'])
            self.logger.info(f"使用模拟数据，共{len(records)}条记录")
        else:
        ...
        for record in sorted_records:
            ...
            # 如果需要重置，清空累计值
            if reset_needed:
                logging.info(f"{reset_reason}，重置累计值")
                accumulated_income = 0.0
                accumulated_tax = 0.0
                accumulated_months = 0
                # 清空所有月度累计器
            ...
            if income_type == 1:
                monthly_income = accum['total_amount'] * 0.8 if accum['total_amount'] > 4000 else accum['total_amount'] - 800
            else:
                monthly_income = accum['total_amount']

            monthly_income = max(0, monthly_income)
            ...
            accumulated_additional = accumulated_additional_deduction * accumulated_months  # 新增
            accumulated_other = accumulated_other_deduction * accumulated_months  # 新增
            accumulated_pension = accumulated_pension_deduction * accumulated_months  # 新增
            accumulated_donation = accumulated_donation_deduction   # 新增 这个后续用到后需要进行优化，这个处理逻辑不对
            accumulated_taxable = max(0, accumulated_income - accumulated_deduction - accumulated_special
                                      - accumulated_additional - accumulated_other - accumulated_pension
                                      - accumulated_donation)
            tax_rate, quick_deduction = self.get_tax_rate_and_deduction(accumulated_taxable)
            accumulated_total_tax = max(0, accumulated_taxable * tax_rate - quick_deduction)

            prev_total_paid_tax = accumulated_tax
            prev_month_paid_tax = accum['paid_tax']
            current_tax = max(0, accumulated_total_tax - prev_total_paid_tax - prev_month_paid_tax)

            accum['paid_tax'] += current_tax
            accumulated_tax += current_tax
            ...
            income_for_display = monthly_income if len(accum['records']) == 1 else monthly_income - (
                    accum['monthly_income'] - record['bill_amount'])

            if len(accum['records']) == 1:
                calculation_steps.append(
                    f"累计收入额: {prev_annual_accumulated_income:.2f} + {monthly_income:.2f} → {accumulated_income:.2f}")
            else:
                prev_month_income = accum['monthly_income'] - record['bill_amount'] if income_type != 1 else accum[
                                                                                                                 'monthly_income'] - (
                                                                                                                     accum[
                                                                                                                         'total_amount'] * 0.8 - prev_month_total_amount * 0.8)
                calculation_steps.append(
                    f"累计收入额: ({prev_annual_accumulated_income:.2f} - {prev_month_income:.2f}) + {monthly_income:.2f} → {accumulated_income:.2f}")

            calculation_steps.extend([
                f"累计月份: {accumulated_months}",
                f"累计减除费用: 5000 × {accumulated_months} = {accumulated_deduction:.2f}",
                f"累计专项扣除: {accumulated_special_deduction} × {accumulated_months} = {accumulated_special:.2f}",
                f"累计专项附加扣除: {accumulated_additional_deduction} × {accumulated_months} = {accumulated_additional:.2f}",
                f"累计其他扣除: {accumulated_other_deduction} × {accumulated_months} = {accumulated_other:.2f}",
                f"累计个人养老金: {accumulated_pension_deduction} × {accumulated_months} = {accumulated_pension:.2f}",
                ...
                f"累计已预缴税额（截至上期）: {prev_total_paid_tax:.2f}",
            ])

            if prev_month_paid_tax > 0:
                calculation_steps.append(
                    f"本次应缴税额: max(0, {accumulated_total_tax:.2f} - {prev_total_paid_tax:.2f} - {prev_month_paid_tax:.2f}) = {current_tax:.2f}")
            else:
                calculation_steps.append(
                    f"本次应缴税额: max(0, {accumulated_total_tax:.2f} - {prev_total_paid_tax:.2f}) = {current_tax:.2f}")

            calculation_steps.append(f"当月已缴税额（含本次）: {accum['paid_tax']:.2f}")
            ...
            result = {
                ...
                'accumulated_additional': round(accumulated_additional, 2),  # 新增
                'accumulated_other': round(accumulated_other, 2),  # 新增
                'accumulated_pension': round(accumulated_pension, 2),  # 新增
                'accumulated_donation': round(accumulated_donation, 2),  # 新增
            ...
            self.logger.info(f"金额={record['bill_amount']:.2f} 税额={current_tax:.2f}")

        total_tax = sum(record['tax'] for record in results)
        self.logger.info(f"年度计算完成: 总税额={total_tax:.2f}元")
