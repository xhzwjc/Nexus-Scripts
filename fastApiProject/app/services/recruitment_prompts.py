...
- If a dimension in DIMENSION_RULES has a "rubric" array, you MUST score strictly according to the rubric levels. The reason must explicitly reference which rubric level the candidate falls into and why, e.g., "候选人有3年经验，落入2.0-2.9分档（3-5年相关经验），给2.5分". Do not deviate from rubric criteria when rubric is provided.
...
N+1. 必须严格按照各维度的评分细则打分，不得脱离细则自由裁量。reason 必须引用评分细则中对应的档位描述，说明为什么落入该档。
评分维度与满分：
1. {维度名}：满分 {N} 分。核心第一优先，{评估重点说明}
   评分细则：
   - {N} 分：{满分标准，如：5年以上同类岗位经验，且有大厂或行业头部公司背景}
   - {N*0.6}-{N*0.9} 分：{中等标准，如：3-5年相关经验，或经验年限足够但行业差异较大}
   - {N*0.1}-{N*0.5} 分：{较低标准，如：1-3年相关经验，或仅有实习/项目经验}
   - 0 分：{不满足标准，如：无相关经验或简历未提及}
...
评分细则设计规则（非常重要）：
- 每个维度都必须包含"评分细则"子段，紧跟在维度描述之后
- 评分细则格式：缩进3空格 + "评分细则："，换行后每个档位缩进3空格 + "- {分值范围} 分：{标准描述}"
- 每个维度设置 3-5 个分档，覆盖从满分到 0 分的完整区间
- 分档之间不得重叠，不得留空隙（如 3.0、2.0-2.9、1.0-1.9、0）
- 档位标准必须具体、可验证，引用 JD 中的具体要求（如年限、技术栈、行业背景）
- 满分标准应列出该维度最优秀候选人应满足的全部条件
- 0 分标准统一为"无相关经验或简历未提及"（除非该维度有特殊逻辑）
- 核心维度和次要维度必须有评分细则；辅助维度和加分项可选
"""
