...
const zhRecruitmentLocale: RecruitmentLocaleBundle = {
    ...
    toast: {
        ...
        noScreeningQueued: "没有成功入队任何初筛任务，请检查候选人是否已关联岗位且岗位已绑定初筛 Skills",
        ...
        screeningQueued: (queued, skipped, failed) => `已入队 ${queued} 个，已跳过进行中的 ${skipped} 个${failed > 0 ? `，失败 ${failed} 个（请检查候选人是否已关联岗位且岗位已绑定初筛 Skills）` : ""}。`,
...
const enRecruitmentLocale: RecruitmentLocaleBundle = {
    ...
    toast: {
        ...
        noScreeningQueued: "No screening tasks were queued successfully. Please check if candidates are linked to a position and the position has screening skills bound.",
        ...
        screeningQueued: (queued, skipped, failed) => `Queued ${queued}, skipped ${skipped} already-running task(s)${failed > 0 ? `, failed ${failed} (please check if candidates are linked to a position and the position has screening skills bound)` : ""}.`,
