...
test("降级口径不展示评分差异，最佳标记遵守人工分语义", () => {
    ...
    assert.match(comparisonWorkspaceSource, /if \(!preview\?\.comparability\.ranking_allowed \|\| preview\.manual_override_mode !== "none"\) return emptyResult/);
...
test("最高分标记区分全员同分、唯一最高和部分并列最高", () => {
    assert.match(apiTypesSource, /is_tied_highest: boolean/);
    assert.match(comparisonWorkspaceSource, /highestCandidates\.length === scoredCandidates\.length/);
    assert.match(comparisonWorkspaceSource, /overallHighest\.isTied \? text\.totalScoreTiedHighest : text\.totalScoreHighest/);
    assert.match(comparisonWorkspaceSource, /value\?\.is_tied_highest \? text\.tiedHighestScore : text\.highestScore/);
    assert.match(containerSource, /highestCandidateIds\.size < normalizedScores\.length/);
    assert.match(containerSource, /preview\.comparability\.ranking_basis === "ai"/);
    for (const source of [i18nTypesSource, zhSource, enSource]) {
        assert.match(source, /totalScoreTiedHighest:/);
        assert.match(source, /tiedHighestScore:/);
    }
});
