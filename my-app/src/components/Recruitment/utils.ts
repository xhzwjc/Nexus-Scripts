 ...
 * AI 招聘模块 - 工具函数
 *
 * 从 RecruitmentAutomationContainer.tsx 提取的纯函数集合。
 * - 统一 Unicode 转义为中文字符串（问题 #17）
 * - 移除硬编码 API key（问题 #18）
 */

import type { AITaskLog, RecruitmentSkill } from "@/lib/recruitment-api";
import {
...
export function joinTags(tags: string[] | undefined | null) {
...
export function normalizeSkillTaskName(value?: string | null) {
...
export function buildLogObjectLabel(
    log: Pick<AITaskLog, "related_position_id" | "related_candidate_id" | "related_skill_id">,
    positionMap: Map<number, { id: number; title: string }>,
    candidateMap: Map<number, { id: number; name: string }>,
    skillMap: Map<number, { id: number; name: string }>
) {
