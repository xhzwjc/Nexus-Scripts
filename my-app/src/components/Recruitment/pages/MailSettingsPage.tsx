...
export interface MailSettingsPageProps {
                ...
                <CardContent className="space-y-4">
                    {resumeMailDispatches.length ? resumeMailDispatches.map((dispatch) => {
                        const recipientSummary = [
                            ...dispatch.recipient_ids.map((recipientId: number) => mailRecipientMap.get(recipientId)?.name || `收件人 #${recipientId}`),
                            ...dispatch.recipient_emails,
                        ].join("、");
                        const candidateSummary = dispatch.candidate_ids
                            .map((candidateId: number) => candidateMap.get(candidateId)?.name || `候选人 #${candidateId}`)
                            .join("、");
