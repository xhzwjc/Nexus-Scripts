...
export default function TaskAutomationScript({ onBack }: TaskAutomationScriptProps) {
  ...
  const getStepsForMode = (mode: string): Array<{key: 'login' | 'sign' | 'get_task_ids' | 'delivery' | 'get_balance_id'; label: string}> => {
    const allSteps: Array<{key: 'login' | 'sign' | 'get_task_ids' | 'delivery' | 'get_balance_id'; label: string}> = [
      { key: 'login', label: '登录' },
      { key: 'sign', label: '报名' },
    ...
    switch (mode) {
      case '1': return allSteps.filter(s => ['login', 'sign'].includes(s.key));
      case '2': return allSteps.filter(s => ['login', 'delivery'].includes(s.key));
      case '3': return allSteps.filter(s => ['login', 'get_balance_id'].includes(s.key));
  ...
  const fetchPhoneNumbers = async () => {
    ...
    try {
      ...
      // 更新手机号列表
      setPhoneNumbers(data.data.map((item: any) => ({
        phone: item.mobile,
        mobile: item.mobile,
        status: 'pending',
        success: false,
  ...
  const getModeDescription = (modeValue: string) => {
    switch (modeValue) {
      case 'full': return '完整流程（登录→报名→提交交付物）';
      case '1': return '登录+报名（mode = 1）';
      case '2': return '登录+提交交付物（mode = 2）';
      case '3': return '登录+结算确认（mode = 3）';
      default: return '';
    }
  };
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <Tabs value={currentTab} onValueChange={setCurrentTab} className="space-y-6">
            <TabsList className="grid w-full grid-cols-4">
              ...
              <TabsTrigger value="results" disabled={executionResults.filter(p => p.status !== 'pending').length === 0}>执行结果</TabsTrigger>
            </TabsList>
                            ...
                            <TableRow>
                              ...
                              <TableCell>
                                ...
                                {phone.status === 'success' && (
                                    <Badge variant="secondary" className="bg-green-100 text-green-800">成功</Badge>
                                )}
                                {phone.status === 'failed' && (
                            ...
                            {phone.status !== 'pending' && (
                                <TableRow>
                                  <TableCell colSpan={3}>
                                    ...
                                    {expandedPhone === phone.mobile && (
                                        <div className="mt-2 p-3 bg-gray-50 rounded-md text-sm space-y-4">
                      ...
                      {executionResults.map((phone, index) => (
                          <React.Fragment key={index}>
                            <TableRow>
                              ...
                              <TableCell>
                                {phone.status === 'success' && (
                                    <Badge variant="secondary" className="bg-green-100 text-green-800">执行成功</Badge>
                                )}
                                {phone.status === 'failed' && (
                                        ...
                                        <div key={key}>
                                          ...
                                          {mode === '3' && key === 'get_balance_id' ? (
                                            <div className="bg-gray-100 p-3 rounded overflow-x-auto">
                                              <p className="mb-1"><span className="font-medium">状态码：</span>{getStepProperty(phone.steps[key], 'code')}</p>
                                              <p className="mb-1"><span className="font-medium">信息：</span>{getStepProperty(phone.steps[key], 'msg') || '无信息'}</p>

                                              <div className="mt-2">
                                                <p className="font-medium mb-1">结算单数据：</p>
                                                <pre className="text-xs">{formatStepData(phone.steps[key], key)}</pre>
                                              </div>
