    ...
    return (
        <div className="glass-card p-5 w-70 shrink-0 self-stretch flex flex-col">
            <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-semibold text-slate-700 dark:text-slate-200 text-sm">{t.quickActions.title}</h3>
            </div>
                    ...
                    {recentScripts.map((recent) => {
                        ...
                        return (
                            <button
                                ...
                                className="w-full flex items-center gap-3 p-2.5 rounded-lg bg-white/50 dark:bg-white/5 hover:bg-white/80 dark:hover:bg-white/10 transition-all group text-left border border-transparent dark:border-white/5"
                            >
                                <div className="p-1.5 bg-teal-50 dark:bg-teal-900/30 rounded-md text-teal-600 dark:text-teal-400">
                                    {script.icon}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-slate-700 dark:text-slate-200 truncate">{translatedName}</p>
                                </div>
                                <Play className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors" />
                            </button>
                        );
                    })}
                </div>
            ...
            {/* 快捷入口区块 */}
            <div className={`space-y-2 ${hasRecent ? 'pt-3 border-t border-slate-100 dark:border-slate-800' : ''}`}>
                <p className="text-xs text-slate-400 mb-2">
                ...
                {quickLinks.map((link) => (
                    <button
                        ...
                        className="w-full flex items-center gap-3 p-2.5 rounded-lg bg-white/50 dark:bg-white/5 hover:bg-white/80 dark:hover:bg-white/10 transition-all group text-left border border-transparent dark:border-white/5"
                    >
                        <div className="p-1.5 bg-slate-50 dark:bg-slate-800 rounded-md text-slate-500 dark:text-slate-400 group-hover:text-teal-600 dark:group-hover:text-teal-400 group-hover:bg-teal-50 dark:group-hover:bg-teal-900/30 transition-colors">
                            {link.icon}
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-slate-600 dark:text-slate-300 group-hover:text-slate-700 dark:group-hover:text-slate-200 truncate">{link.label}</p>
                        </div>
                        <ArrowRight className="w-3.5 h-3.5 text-slate-300 dark:text-slate-600 group-hover:text-teal-500 dark:group-hover:text-teal-400 transition-colors" />
                    </button>
                ))}
            </div>
